...
var connectionString = {
    ...
    options: { encrypt       : true,
               ...
               appName       : 'scan.datasaturday.no' // host name of the web server
        }
    };
        ...
        sqlQuery(connectionString, 'EXECUTE Scan.New_Scan @ID=@ID, @ReferenceCode=@ReferenceCode;',
            ...
            async function(recordset) {
                if (recordset.length==1) {
                    ...
                    res.status(500).send(createHTML('assets/error.html', { "Msg": recordset.err + "That code didn't look right." }));
                    return;
                }
            });
    } catch(e) {
